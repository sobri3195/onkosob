...
export function PublicLayout({ onAdmin }: { onAdmin: () => void }) {
  ...
  const [open, setOpen] = useState(false); const [scrolled, setScrolled] = useState(false); const [onboarding,setOnboarding]=useState(!profile.onboardingCompleted); const [palette,setPalette]=useState(false); const location = useLocation()
  ...
  const dismiss=()=>setOnboarding(false)
  ...
  return <div className='site-frame'>
    <header className={`site-header ${scrolled ? 'is-scrolled' : ''}`}>
      ...
      <div className={`mobile-menu ${open ? 'open' : ''}`} aria-hidden={!open}><p>JELAJAHI LENTERA</p>{navItems.map((item, i) => <Link key={item.label} to={item.to} onClick={() => go(item.to)}><span>0{i + 1}</span>{item.label}<ArrowRight /></Link>)}<Link to='/today' onClick={() => setOpen(false)}>Pendamping Harian <ArrowRight/></Link><Link to='/saved' onClick={() => setOpen(false)}>Tersimpan <ArrowRight/></Link><Link to='/dashboard' className='button button-primary' onClick={() => setOpen(false)}>Dashboard</Link></div>
    </header>
    <main><Outlet /></main>
    {onboarding&&location.pathname!=='/dashboard'&&<aside className='onboarding-panel'><div><strong>Temukan informasi sesuai perjalanan Anda</strong><p>Lentera dapat membantu Anda menemukan informasi berdasarkan tahap perjalanan yang sedang Anda jalani.</p></div><Link to='/dashboard' onClick={dismiss}>Mulai</Link><button onClick={dismiss}>Jelajahi sendiri</button></aside>}
