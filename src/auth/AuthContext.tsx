...
import { sessionStore, supabase, type Session, type SessionUser } from '@/lib/supabase'
...
export function AuthProvider({children}:{children:React.ReactNode}){const [session,setSession]=useState<Session|null>(()=>sessionStore.get()),[profile,setProfile]=useState<Profile|null>(null),[loading,setLoading]=useState(true)
 ...
 useEffect(()=>{let active=true;(async()=>{const current=sessionStore.get();if(current){const refreshed=await supabase.auth.refresh();if(active)setSession(refreshed);if(refreshed)await refreshProfile()}if(active)setLoading(false)})();return()=>{active=false}},[refreshProfile])
